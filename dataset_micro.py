...
def jepa_micro_collate_fn(batch):
    x = torch.stack(batch, dim=0)
    B = x.shape[0]
    total_patches = 100 # 5000 / 50 = 100 yamacik
    
    # Atım-içi dinamikleri bozmak icin rastgele 3 farkli noktadan 2-3 yama (0.1s - 0.15s) gizle
    tgt_idx_set = set()
    num_blocks = 3
    
    for _ in range(num_blocks):
        tgt_len = random.choice([2, 3])
        start_idx = random.randint(0, total_patches - tgt_len)
        for i in range(start_idx, start_idx + tgt_len):
            tgt_idx_set.add(i)
            
    # Tensor boyutlarinin sabit kalmasi icin 8 tane yamayi sabitleyelim (batch icinde boyut degismesin)
    # Eger 8'den azsa tamamlayalim, coksa keselim
    tgt_list = sorted(list(tgt_idx_set))
    if len(tgt_list) > 8:
        tgt_list = tgt_list[:8]
    elif len(tgt_list) < 8:
        # Eksikse baska bos yerlerden ekle
        all_possible = set(range(total_patches))
        avail = list(all_possible - set(tgt_list))
        random.shuffle(avail)
        tgt_list.extend(avail[:8 - len(tgt_list)])
        tgt_list.sort()
        
    tgt_idx = torch.tensor(tgt_list).unsqueeze(0).repeat(B, 1)
    
    avail_ctx = [i for i in range(total_patches) if i not in tgt_list]
